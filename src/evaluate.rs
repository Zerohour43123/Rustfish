...
fn evaluate_pieces<Us: ColorTrait, Pt: PieceTypeTrait>(
    ...
    let them = if us == WHITE { BLACK } else { WHITE };
    let outpost_ranks =
        if us == WHITE { RANK4_BB | RANK5_BB | RANK6_BB } else { RANK5_BB | RANK4_BB | RANK3_BB };
...
fn evaluate_king<Us: ColorTrait>(pos: &Position, ei: &mut EvalInfo) -> Score {
    let us = Us::COLOR;
    let them = if us == WHITE { BLACK } else { WHITE };
    let camp = if us == WHITE { ALL_SQUARES ^ RANK6_BB ^ RANK7_BB ^ RANK8_BB } else { ALL_SQUARES ^ RANK1_BB ^ RANK2_BB ^ RANK3_BB };
    ...
    for s in ei.pe.passed_pawns(us) {
        ...
        if rr != 0 {
            ...
            if pos.empty(block_sq) {
                ...
                // attacked.
                let mut k = if unsafe_squares == 0 { 20 } else if unsafe_squares & block_sq == 0 { 9 } else { 0 };
...
fn evaluate_scale_factor(
    ...
    if sf == ScaleFactor::NORMAL || sf == ScaleFactor::ONEPAWN {
        if pos.opposite_bishops() {
            ...
            {
                return if more_than_one(pos.pieces_p(PAWN)) { ScaleFactor(31) } else { ScaleFactor(9) };
            }
